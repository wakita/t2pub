...
function submit(spec) {
  ...
  if (spec.log && spec.log.argv) spec.PBS.argv = argv;
...
function mpirun(spec) {
  ...
  var argv = [
    ...
    spec.Command ];
  if (spec.argv) argv.push.apply(argv, spec.argv);

  if (spec.log && spec.log.argv) MPI.argv = argv;
  if (spec.log && spec.log.env) MPI.env = env;
  fs.writeFileSync(spec.Runconf, JSON.stringify(spec, null, 2));
